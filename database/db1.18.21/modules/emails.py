...
def getEmails(listorfile,listofdepts,perc):
    import re
    if type(listorfile)==list:
        deptlist=listorfile
        total=len(deptlist)
        num=perc*total
        deptlistr = revlist(deptlist[0:num])
        dept_emailstext = ''.join(emails.getEmailsLB(deptlistr))
        return dept_emailstext
    elif type(listorfile)==str:
        from urllib import request
        txtfile=listorfile
        f = open(txtfile, "r+")
        deptstr = f.read()
        deptregex=getreg(listofdepts)
        #deptregex = str(dept + ".*txt") # e.g. "executiveofficeemails.*"
        deptlist = re.findall(deptregex, deptstr)
        deptlistd=list(dict.fromkeys(deptlist))
        total=len(deptlistd)
        num=int(perc*total)
        deptlistr = revlist(deptlistd[0:num])
        dept_emailstext = ''.join(getEmailsLB(deptlistr))
        return dept_emailstext
        #return num,total,deptlist
